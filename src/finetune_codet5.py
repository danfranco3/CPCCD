...
embedding_model = AutoModel.from_pretrained(CHECKPOINT, trust_remote_code=True).to(DEVICE)
...
def get_embedding(code):
    ...
    with torch.no_grad():
        emb = embedding_model(inputs.input_ids)[0]  # shape: [1, 256]
    return emb.squeeze(0).cpu()  # shape: [256]
...
for epoch in range(EPOCHS):
    ...
    for code1s, code2s, labels in train_loader:
        emb1s = torch.stack([get_embedding(c) for c in code1s]).to(DEVICE)
        emb2s = torch.stack([get_embedding(c) for c in code2s]).to(DEVICE)
