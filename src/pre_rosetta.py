...
def build_positive_pairs(df1, df2):
...
def build_negative_pairs(df1, df2, task_pool, num_pairs):
    negatives = []
...
def generate_clone_dataset(df, lang1, lang2, test_size=0.3, seed=42):
    ...
    train_pos = build_positive_pairs(train_df1, train_df2)
    test_pos = build_positive_pairs(test_df1, test_df2)

    train_neg = build_negative_pairs(train_df1, train_df2, train_tasks, len(train_pos))
    test_neg = build_negative_pairs(test_df1, test_df2, test_tasks, len(test_pos))
...
def main():
    train, test = generate_clone_dataset(df, 'Java', 'Fortran', test_size=0.3)
    save_dataset(train, test, prefix='java_fortran_clones')

    train, test = generate_clone_dataset(df, 'Python', 'COBOL', test_size=0.3)
    save_dataset(train, test, prefix='python_cobol_clones')

    train, test = generate_clone_dataset(df, 'JavaScript', 'Free Pascal', test_size=0.3)
